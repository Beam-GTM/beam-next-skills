...
**Key Principle**:
- `02-memory/input/` is TEMPORARY - holds uploads until organized
- SubAgents write to `02-memory/input/_analysis/` (also temporary)
- The "Organize Initial Context" PROJECT distributes everything to `04-workspace/`
- After organization, `02-memory/input/` gets cleaned up
...
**Create folders**:
```bash
mkdir -p 02-memory/input/           # Where user uploads files (temporary)
mkdir -p 02-memory/input/_analysis/ # Where analysis results go (temporary)
```
...
**Use Glob tool** to scan `02-memory/input/*`:

```python
from pathlib import Path

input_dir = Path("02-memory/input/")
uploaded_files = [f for f in input_dir.glob("*") if f.is_file()]

if not uploaded_files:
    print("No files found in 02-memory/input/")
...
```
User uploads to 02-memory/input/:
  report.pdf (80 pages)
  notes.docx
  chat.json
...
EXTRACTED_DIR = Path("02-memory/input/_analysis/extracted")
...
[
  "02-memory/input/_analysis/extracted/report_chunk01.txt",
  "02-memory/input/_analysis/extracted/report_chunk02.txt",
  ...
]
```
...
For each file in `02-memory/input/`:
...
# Example result:
processable_files = [
    Path("02-memory/input/_analysis/extracted/report_chunk01.txt"),
    Path("02-memory/input/_analysis/extracted/report_chunk02.txt"),
    Path("02-memory/input/_analysis/extracted/report_chunk03.txt"),
    Path("02-memory/input/_analysis/extracted/notes.txt"),
    Path("02-memory/input/chat.json"),  # small, kept as-is
    Path("02-memory/input/screenshot.png"),  # image, kept as-is
]
```
...
3. Writes to `02-memory/input/_analysis/batch-{N}-insights.md`
...
3. Write a STRUCTURED DOCUMENT to: 02-memory/input/_analysis/batch-{batch_number:02d}-insights.md
...
Write to `02-memory/input/_analysis/batch-{batch_number:02d}-insights.md`:
...
**Potential PROJECT Ideas**: {ideas from this file}
...
**PROJECT Ideas**: {aggregated list with source file}
...
1. **Each agent wrote**: `02-memory/input/_analysis/batch-{N}-insights.md`
...
```python
from pathlib import Path
from beam_next.validation.validators import validate_aggregation_completeness

analysis_dir = Path("02-memory/input/_analysis/")
...
if expected_batch_count > 3:
    ...
    Task(
        subagent_type="general-purpose",
        prompt=f"""
        Aggregate all batch analysis files in 02-memory/input/_analysis/.
        ...
        Use beam_next.validators.validate_aggregation_completeness() to validate:
        - After discovering files
        - After reading all files
        - Before writing summary

        Output: 02-memory/input/_analysis/analysis-summary.md
        """,
...
**Create master summary**: `02-memory/input/_analysis/analysis-summary.md`
...
- **{tool}**: {how it could integrate with Beam Next}
...
Saved to:
→ 02-memory/input/_analysis/analysis-summary.md (master)
→ 02-memory/input/_analysis/batch-{N}-insights.md (per batch)
...
for file, target_folder in organization_plan.items():
    source = Path(f"02-memory/input/{file}")
...
```
Okay, files stay in 02-memory/input/.
...
roadmap_path = Path("02-memory/roadmap.md")
```
...
This project sorts them into your workspace where they become permanent,
organized context that informs all future work.

**What "Done" Looks Like**:
- Files moved from 02-memory/input/ to 04-workspace/{folders}/
- Key insights saved to 04-workspace/references/
- Workspace-map.md updated with new structure
- 02-memory/input/ cleared (ready for next upload)
```
...
This will be your first project - or you can do it after your current focus.
```
...
**All outputs are TEMPORARY until "Organize Initial Context" PROJECT runs.**

```
02-memory/input/              # TEMPORARY - cleared after organization
...
**After "Organize Initial Context" PROJECT**:
```
...
- PROJECT ideas inform Step 4

The AI should reference `02-memory/input/_analysis/analysis-summary.md` in subsequent steps.
