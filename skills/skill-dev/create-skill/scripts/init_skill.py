...
4. Skill appears in "Beam Next Skills" database with full .skill file attached
5. Teammates can query and import with `query-notion-db` and `import-skill-to-beam-next`
...
def get_skill_path(skill_name, path=None):
    """
    Get skill path, auto-detecting Beam Next structure if path not provided.
    """
    if path:
        return Path(path).resolve() / skill_name

    # Auto-detect Beam Next structure
    cwd = Path.cwd()

    # Check if running in Beam Next context
    beam_next_user_skills = cwd / "01-skills"
    beam_next_system_skills = cwd / "00-system" / "skills"

    if beam_next_user_skills.exists() and beam_next_user_skills.is_dir():
        return beam_next_user_skills / skill_name
    elif beam_next_system_skills.exists() and beam_next_system_skills.is_dir():
        return beam_next_system_skills / skill_name
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Initialize a new Beam Next skill with template structure',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  init_skill.py my-new-skill                     # Auto-detect location
  init_skill.py my-skill --path 01-skills        # Explicit path
