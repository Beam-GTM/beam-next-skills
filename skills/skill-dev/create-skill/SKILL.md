...
After understanding the skill requirements, AI **runs** `uv run beam-next-mental-models --format brief` to see available thinking frameworks, then offers 2-3 relevant options to deepen skill design.
...
**Optional: Beam Next Integration**

If using Beam Next framework: Skills placed in `01-skills/` (user) or `00-system/skills/` (system) auto-integrate. The framework's loader scans YAML metadata on startup—no manual registration needed. Skills work standalone or in Beam Next without modification.
...
5. **Teammates can discover it** using `query-notion-db` and import using `import-skill-to-beam-next`
...
- See [import-skill-to-beam-next](../import-skill-to-beam-next/SKILL.md) for importing teammate skills
...
```bash
beam-next-load --skill close-session
```
