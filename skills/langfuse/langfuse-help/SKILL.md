...
```bash
# 1. Check config
uv run python 01-skills/langfuse/langfuse-master/scripts/check_langfuse_config.py --test

# 2. List recent traces
uv run python 01-skills/langfuse/langfuse-list-traces/scripts/list_traces.py --limit 10

# 3. Get trace with observations
uv run python 01-skills/langfuse/langfuse-get-trace/scripts/get_trace.py --id {trace_id}

# 4. Create score (validated)
uv run python 01-skills/langfuse/langfuse-create-score/scripts/create_score.py \
