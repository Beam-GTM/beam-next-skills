#!/usr/bin/env python3
"""
Build Initializer - Creates a new Beam Next build from template

Usage:
    init_project.py <build-name> --path <path>

Examples:
    init_project.py website-redesign --path Builds
    init_project.py client-portal --path Builds
    init_project.py "Marketing Campaign" --path Builds
...
created: {date}
project_path: 03-projects/active/{project_id}-{sanitized_name}/
---
...
**Related Builds**:
- [To be filled during discovery - use paths like `03-projects/XX-name/`]
...
**Build Location**: `03-projects/active/{project_id}-{sanitized_name}/`
...
def load_type_template(build_type, template_name):
    ...
    Args:
        build_type: One of the 8 project types (build, integration, etc.)
...
def get_next_project_id(builds_path):
    """
    Scan the projects directory to determine the next available build ID.
    """
...
def init_project(project_name, path, build_type='generic', description='', project_id_override=None):
    """
    Initialize a new project directory with all planning files from templates.

    Args:
        project_name: Human-readable build name
        ...
        project_id_override: Optional build ID (auto-assigned if not provided)
    """
    ...
    build_dirname = f"{project_id}-{sanitized_name}"
    ...
    try:
        ...
        print(f"[OK] Created project directory: {build_dir}")
    except Exception as e:
...
"""
    ...
    print(f"\n[SUCCESS] Build '{project_name}' initialized successfully!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Initialize a new Beam Next build with templates",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  init_project.py "Website Redesign" --path 03-projects/active --type build
  init_project.py "Market Research" --path 03-projects/active --type research
  init_project.py my-feature --path 03-projects/active -d "Add new feature" --id 15
...
"""
    ...
    parser.add_argument("--path", "-p", default="03-projects/active",
                        help="Path to builds directory (default: 03-projects/active)")
