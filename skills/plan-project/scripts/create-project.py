...
This script is deprecated. Use the new improved script:
    python 00-system/skills/plan-project/scripts/init_project.py "Build Name" --path 03-projects/active
...
- Aligned with plan-project skill's interactive planning mode
...
plan-project.py - Initialize a new project with proper structure and YAML frontmatter

Usage:
    python plan-project.py <build-name> --id <number>
    python plan-project.py website-development --id 05

Example:
    python plan-project.py client-proposal-system --id 06
"""
...
print("="*70)
print("This script (plan-project.py) is deprecated.")
print("\nPlease use the new script instead:")
print("  python 00-system/skills/plan-project/scripts/init_project.py \"Build Name\" --path 03-projects/active")
...
**Note**: This project was created using the `plan-project` script.
...
def create_build(project_name: str, project_id: str, base_path: str = "."):
    """
    Create a new project with structure and templates.

    Args:
        project_name: Kebab-case build name
        project_id: Two-digit build ID
    ...
    """
    base = Path(base_path)
    build_folder = f"{project_id}-{project_name}"
    project_path = base / "02-Builds" / build_folder

    # Check if already exists
    if project_path.exists():
        print(f"[ERROR] Build already exists: {project_path}")
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Create a new Beam Next build")
    ...
    parser.add_argument('--base-path', default='.', help='Base path to Beam Next')
