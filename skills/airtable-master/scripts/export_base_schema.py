...
Example:
    ...
    python export_base_schema.py appFPoOfBpUv73M5A --output 02-memory/integrations/airtable/my-base.yaml
"""
...
if not BEAM_NEXT_ROOT:
    print("Error: Could not find Beam Next root")
    sys.exit(1)

sys.path.insert(0, BEAM_NEXT_ROOT)
...
def load_env():
    """Load .env file."""
    env_path = os.path.join(BEAM_NEXT_ROOT, '.env')
...
def main():
    ...
    if args.output:
        ...
        output_dir = os.path.join(BEAM_NEXT_ROOT, '02-memory', 'integrations', 'airtable', slugify(base_name))
