...
def find_beam_next_root():
    """Find Beam Next root directory (contains CLAUDE.md)"""
...
def load_env(beam_next_root=None):
    """Load .env file into environment variables."""
    if beam_next_root is None:
        beam_next_root = find_beam_next_root()
    if beam_next_root is None:
        return

    env_path = beam_next_root / '.env' if isinstance(beam_next_root, Path) else Path(beam_next_root) / '.env'
...
def get_cache_file_path(beam_next_root=None) -> Path:
    """Get path to airtable-bases.yaml cache file."""
    if beam_next_root is None:
        beam_next_root = find_beam_next_root()
    if beam_next_root is None:
        raise ValueError("Could not find Beam Next root")

    return Path(beam_next_root) / '02-memory' / 'integrations' / 'airtable-bases.yaml'


def load_base_cache(beam_next_root=None) -> dict:
    """Load cached base info from YAML file."""
    cache_file = get_cache_file_path(beam_next_root)
...
def update_context_file(beam_next_root=None, bases=None, base_update=None):
    """
    Update airtable-bases.yaml context file.

    Args:
        beam_next_root: Path to Beam Next root
    ...
    if beam_next_root is None:
        beam_next_root = find_beam_next_root()
    if beam_next_root is None:
        print("[WARN] Could not find Beam Next root, skipping context update", file=sys.stderr)
        return
...
_beam_next_root = find_beam_next_root()
if _beam_next_root:
    BEAM_NEXT_ROOT = _beam_next_root
    CACHE_FILE = get_cache_file_path(_beam_next_root)
else:
    BEAM_NEXT_ROOT = None
    CACHE_FILE = None
