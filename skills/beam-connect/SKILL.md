...
**Batch Retry** (all failed tasks from an agent):
```bash
python 00-system/skills/beam/beam-master/scripts/retry_task.py --agent-id AGENT_ID --statuses FAILED,STOPPED --dry-run
python 00-system/skills/beam/beam-master/scripts/retry_task.py --agent-id AGENT_ID --statuses FAILED,STOPPED
```
...
| Skill | When to Use |
|-------|-------------|
| `deploy-to-beam` | Deploy Beam Next skills as Platform agents |
| `design-beam-agent` | Design agent node architecture from scoping doc |
| `calculate-beam-agent-pricing` | Agent pricing & economics calculations |
| `beam-feedback-automation` | Feedback collection via Google Sheets |
| `beam-design-system` | Beam design tokens (colors, typography, components) |

All operational tasks (list agents, get graphs, analytics, tasks, retries, node editing) are handled
directly by this skill via `beam-master` scripts. No need for separate skills.
