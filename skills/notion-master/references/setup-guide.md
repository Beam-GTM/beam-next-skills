...
**Single source of truth for setting up Notion integration with Beam Next**
...
Step 1: Get API Key (choose one option)
   ...
   Option B: Create your own (admin only)
      ...
      → Click "New integration" → Name: "Beam Next"
...
Step 3: Connect Database (if using your own key)
   → Open "Beam Beam Next Skills" database in Notion
...
**Location:** Project root (`Beam-Next-v4/.env`)
...
**Location:** `02-memory/user-config.yaml`
...
2. Navigate to "Beam Beam Next Skills" database
