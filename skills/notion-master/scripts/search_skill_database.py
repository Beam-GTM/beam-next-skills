...
Usage:
    ...
    # Skills DB preset mode (for Beam Beam Next Skills database):
...
def load_context_file(root_path):
    ...
    """
    context_file = root_path / "02-memory" / "integrations" / "notion-databases.yaml"
...
Examples:
  ...
  python search_skill_database.py --skills --integration "Beam AI" --name notion
    ...
    parser.add_argument("--skills", action="store_true", help="Query Beam Beam Next Skills database (preset mode)")
    ...
    if args.skills:
        ...
        print(f"[INFO] Querying: Beam Beam Next Skills (preset mode)", file=sys.stderr)
        if filters:
