...
TEST_CHANNEL_PREFIX = "beam-next-test-"
...
class SlackTestSuite:
    ...
    def test_set_channel_topic(self, result):
        ...
        response = self.client.post('conversations.setTopic', {
            ...
            'topic': 'Beam Next Integration Test Channel - Safe to delete'
        })
        if response.get('ok'):
    ...
    def test_create_reminder(self, result):
        ...
        response = self.client.post('reminders.add', {
            'text': 'Beam Next test reminder - safe to delete',
            'time': reminder_time
        })
    ...
    def test_upload_file(self, result):
        ...
        content = 'This is a test file from Beam Next Slack Integration Test.\nSafe to delete.'
        filename = 'beam_next_test_file.txt'
        ...
        complete_response = self.client.post('files.completeUploadExternal', {
            'files': [{'id': file_id, 'title': 'Beam Next Test File'}],
    ...
    def cleanup(self):
        ...
        try:
            self.client.post('chat.postMessage', {
                'channel': self.test_dm_channel,
                'text': "✅ *Beam Next Slack Integration Test Complete*\n\nAll test artifacts have been cleaned up."
            })
        except:
            pass
    ...
    def run(self):
        ...
        self.log("\n" + "=" * 60)
        self.log("  BEAM NEXT SLACK INTEGRATION TEST SUITE")
        self.log("=" * 60)
