#!/usr/bin/env python3
"""
Workable Master Client
All Workable ATS operations: fetch JDs, search candidates, post assessments/reviews.

Usage:
    python3 workable_client.py --fetch-jd "Product Manager"
    python3 workable_client.py --fetch-jd-code 435CFC6C38
    python3 workable_client.py --search "Sarah"
    python3 workable_client.py --candidate "Sarah Chen" --assessment "text"
    python3 workable_client.py --candidate "Sarah Chen" --review --grade 1 --assessment "text"
    python3 workable_client.py --list-jobs
    python3 workable_client.py --find-candidate-in-job 435CFC6C38 --candidate "Usama"
"""
...
def fetch_jd(query, api_key, subdomain, by_code=False):
    """Fetch a job description from Workable by title search or shortcode."""
    if by_code:
        print(f"\n📄 Fetching JD for shortcode '{query}'...")
        result = api_request("GET", f"/jobs/{query}", api_key, subdomain)
        jobs = [result] if result else []
    else:
        print(f"\n📄 Searching jobs matching '{query}'...")
        result = api_request("GET", "/jobs?state=published&limit=50", api_key, subdomain)
        all_jobs = result.get("jobs", [])
        query_lower = query.lower()
        jobs = [j for j in all_jobs if query_lower in j.get("title", "").lower()]
        if not jobs:
            result2 = api_request("GET", "/jobs?state=closed&limit=50", api_key, subdomain)
            all_closed = result2.get("jobs", [])
            jobs = [j for j in all_closed if query_lower in j.get("title", "").lower()]

    if not jobs:
        print(f"   No jobs found matching '{query}'.")
        return None

    if len(jobs) > 1 and not by_code:
        print(f"   Found {len(jobs)} matches:\n")
        for j in jobs:
            print(f"   • {j['title']} [{j.get('shortcode','')}] — {j.get('department','')}")
        print(f"\n   Use --fetch-jd-code [shortcode] for a specific job.")
        return None
    ...
    if not by_code:
        full_job = api_request("GET", f"/jobs/{shortcode}", api_key, subdomain)
    else:
        full_job = job
    ...
    department = full_job.get("department", "")
    location = full_job.get("location", {}).get("city", "Remote")
    desc = clean_whitespace(strip_html(full_job.get("full_description", "") or full_job.get("description", "")))
    reqs = clean_whitespace(strip_html(full_job.get("requirements", "")))
    benefits = clean_whitespace(strip_html(full_job.get("benefits", "")))

    print(f"\n   Title: {title} [{shortcode}]")
    print(f"   Department: {department} | Location: {location}")
    print(f"   Description: {desc[:200]}...")
    if reqs:
        print(f"   Requirements: {reqs[:200]}...")
...
def find_candidate_in_job(job_shortcode, name, api_key, subdomain):
    """Search for a candidate within a specific job's pipeline."""
    print(f"\n🔍 Searching '{name}' in job {job_shortcode}...")
    base = f"https://{subdomain}.workable.com/spi/v3"
    matches = []
    name_lower = name.lower()
    next_url = f"{base}/jobs/{job_shortcode}/candidates?limit=100"
    pages = 0
    ...
    if not matches:
        print(f"   No candidates matching '{name}' in this job.")
        return []

    print(f"   Found {len(matches)} match(es):\n")
    for i, m in enumerate(matches, 1):
        print(f"   {i}. {m['name']} — {m['stage']}")
        print(f"      Email: {m['email']} | ID: {m['id']}")
    return matches
...
def main():
    parser = argparse.ArgumentParser(description="Workable Master Client — JDs, candidates, assessments, reviews")

    group = parser.add_mutually_exclusive_group()
    group.add_argument("--fetch-jd", metavar="TITLE", help="Fetch JD by job title search")
    group.add_argument("--fetch-jd-code", metavar="CODE", help="Fetch JD by job shortcode")
    group.add_argument("--search", help="Search for candidate by name globally")
    group.add_argument("--list-jobs", action="store_true", help="List open jobs")

    parser.add_argument("--find-candidate-in-job", metavar="CODE", help="Search candidate within a specific job shortcode")
    parser.add_argument("--candidate", help="Candidate name to search for (for posting)")
    ...
    parser.add_argument("--review", action="store_true", help="Post as formal review (thumbs rating)")
    parser.add_argument("--grade", type=int, choices=[0, 1, 2], help="Review grade: 0=no, 1=maybe, 2=yes")
    ...
    if args.list_jobs:
        result = list_jobs(api_key, subdomain)
        if args.json and result:
            print(json.dumps(result, indent=2))
        return

    if args.fetch_jd:
        result = fetch_jd(args.fetch_jd, api_key, subdomain)
        if args.json and result:
            print(json.dumps(result, indent=2))
        return

    if args.fetch_jd_code:
        result = fetch_jd(args.fetch_jd_code, api_key, subdomain, by_code=True)
        if args.json and result:
            print(json.dumps(result, indent=2))
        return
    ...
    if args.find_candidate_in_job and args.candidate:
        matches = find_candidate_in_job(args.find_candidate_in_job, args.candidate, api_key, subdomain)
        if args.json:
            print(json.dumps(matches, indent=2))
        return
