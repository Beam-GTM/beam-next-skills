...
**Creative ways to use MCP integrations in Beam Next workflows**
...
**Core Value**: Turn Beam Next from a standalone system into a central hub that connects all your tools.

**Benefits**:
- **Less Manual Work**: Automate data flow between tools
- **Live Data**: Always current, never stale
- **Single Interface**: Control everything from Beam Next
...
- "Start new project for website redesign"
  - Creates Beam Next build structure
...
- Backup 02-memory/core-learnings.md
- Backup 03-projects/build-map.md
...
1. **Issue Generation**
   ```
   Beam Next task list → GitHub issues
   - Parse tasks.md checkboxes
   - Create GitHub issue for each
   - Link back in Beam Next
...
1. **Daily Standups**
   ```
   Morning: Query Beam Next for today's plan
...
2. **Knowledge Base**
   ...
   - All Beam Next skills in Notion
   - Searchable database
...
3. **Meeting Notes**
   ```
   Beam Next session → Meeting note
...
2. **Template Library**
   ...
   - Applies to Beam Next build
   - Updates library as needed
   ```

3. **Collaboration**
   ```
   Draft in Beam Next → Edit in Docs → Finalize in Beam Next
   - Initial draft in Beam Next
...
2. **Sprint Planning**
   ```
   Plan sprint in Beam Next → Push to Linear
   - Break down work
...
4. **Milestone Tracking**
   ```
   Linear milestones → Beam Next build phases
   - Link milestones
   - Track progress
   - Update stakeholders
...
1. **Memory Sync**
   ```
   Beam Next Memory/ ↔ Obsidian vault
   - Keep perfectly synced
   - Use Obsidian for browsing
   - Use Beam Next for workflows
   - Best of both worlds
   ```

2. **Graph Visualization**
   ```
   Visualize Beam Next structure in Obsidian graph
   - Builds as nodes
   - Skills as nodes
...
3. **Daily Notes Integration**
   ```
   Beam Next session → Obsidian daily note
...
4. **Zettelkasten Method**
   ```
   Atomic notes in Beam Next → Obsidian Zettelkasten
   - Create atomic notes
   - Sync to Obsidian
...
```
1. Brainstorm in Beam Next (chat-based)
   ↓
2. Push to Notion content calendar
   ↓
3. Draft in Beam Next with AI assistance
   ↓
4. Export to Google Docs for editing
   ↓
5. Final version back to Beam Next
   ↓
6. Publish (via integration)
   ↓
7. Track performance (via integration)
   ↓
8. Learn and iterate (documented in Beam Next)
```
...
1. User reports issue (Slack)
   ↓
2. Create Beam Next build for investigation
   ↓
...
5. Create Linear issue with findings
   ↓
6. Track fix in Beam Next build
   ↓
...
2. Query Linear for week's completed tasks
   ↓
3. Review Beam Next session reports
   ↓
...
- Extend Beam Next in ways we haven't imagined
...
- **Beam Next Documentation**: See 02-memory/core-learnings.md for your specific setups
