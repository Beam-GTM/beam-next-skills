...
def prepare_graph_for_put(graph):
    """Clean a raw GET response for PUT. Ensures required fields exist."""
...
def summarize_graph(nodes):
    """Produce a summary list of nodes for display."""
    summary = []
    for i, node in enumerate(nodes):
        ...
        summary.append({
            ...
            "input_count": len(tc.get('inputParams', [])),
            "output_count": len(tc.get('outputParams', [])),
        })
    return summary
...
def main():
    ...
    try:
        ...
        # Push
        endpoint = f'/agent-graphs/{args.agent_id}'
        if args.save_and_publish:
            endpoint += '?saveAndPublish=true'

        if not args.json:
            print(f"Pushing to PUT /agent-graphs/{args.agent_id}...")

        result = client.put(endpoint, data=payload)
