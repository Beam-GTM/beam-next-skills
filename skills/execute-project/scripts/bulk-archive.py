...
Purpose:
    Quickly mark tasks in a project's task file as complete.
...
Version 2.1 Changes:
    ...
    - Prefers numbered format (04-steps.md) created by init_project.py
...
def bulk_complete_tasks(
    ...
    Args:
        project_id: Build identifier (e.g., "01-first-build" or just "01")
        base_path: Base path to Beam Next
    ...
    # Find project folder - try multiple patterns (active first, then complete, then root for legacy)
    possible_folders = [
        base / "03-projects" / "active" / project_id,  # Full ID in active
        base / "03-projects" / "active" / f"{project_id.zfill(2)}-*",  # Just number in active
        base / "03-projects" / "archive" / project_id,  # Full ID in complete
        base / "03-projects" / "archive" / f"{project_id.zfill(2)}-*",  # Just number in complete
        base / "03-projects" / project_id,  # Legacy: root of 02-builds
        base / "03-projects" / f"{project_id.zfill(2)}-*",  # Legacy: root with number
    ]
    ...
    if not project_path or not project_path.exists():
        print(f"[ERROR] Build not found: {project_id}")
        print(f"[INFO] Searched in: 03-projects/active/, 03-projects/complete/, 03-projects/")
        return False
    ...
    print(f"Build: {project_path.name}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Bulk complete tasks in a project (all or partial) - V2.1 with 04-steps.md support",
        epilog="""
Examples:
...
Version 2.1 Features:
  ...
  - Backward compatible with all existing builds
    ...
    parser.add_argument(
        '--build',
        required=True,
        help='Project ID (e.g., 01-first-build or just 01)'
    )
    parser.add_argument(
        '--all',
        action='store_true',
    ...
    parser.add_argument(
        '--base-path',
        default='.',
        help='Base path to Beam Next (default: current directory)'
    )
    parser.add_argument(
        '--no-confirm',
        action='store_true',
