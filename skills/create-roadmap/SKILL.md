...
- Transform vague goals into concrete PROJECT items
- Prioritize by impact and dependencies
- Create a living document that guides your Beam Next work
...
| Type | Description |
|------|-------------|
| PROJECT | Finite work with deliverables |
...
- **project_id** and **completed_at**: Always set to `null` initially (system manages these)
...
- If `02-memory/input/_analysis/analysis-summary.md` exists:
  - Use PROJECT ideas from analysis
  - Reference tools for INTEGRATION suggestions
  - Incorporate patterns discovered

**With builds (BIDIRECTIONAL SYNC)**:
- **Forwards**: When you say "build {name}", plan-project matches against roadmap items by slug comparison and sets `project_id`
...
- Status is DERIVED (never stored): `completed_at` set → done; `project_id` in active → in_progress; no `project_id` → not started
