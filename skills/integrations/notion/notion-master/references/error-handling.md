...
### import-skill-to-beam-next Errors
...
|-------|-------|----------|
| `.env` file not found | Missing in Beam Next root | Create `.env` with `NOTION_API_KEY` and `NOTION_SKILLS_DB_ID` |
...
```json
{
  "object": "database",
  "id": "2bc2cadf...",
  "title": [{"plain_text": "Beam Beam Next Skills"}]
}
```
