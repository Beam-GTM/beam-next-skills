...
def check_user_config():
    ...
    user_config_path = BEAM_NEXT_ROOT / '02-memory' / 'user-config.yaml'
...
def build_json_output(env_result, user_result, api_result, env_ok, user_ok, api_ok):
    ...
    if 'NOTION_SKILLS_DB_ID' in env_result.get('missing', []) or not env_result.get('exists'):
        fix_instructions.append({
            'step': 2,
            'action': 'Add Database ID (Beam Beam Next Skills)',
            'details': [
                'Use the default Beam Beam Next Skills database ID:',
...
def main():
    ...
    if all_ok and user_ok:
        ...
        print("  • import-skill-to-beam-next - Import skills from Notion")
        ...
        print()
        print("To enable export, add notion_user_id to 02-memory/user-config.yaml")
        print("="*60)
        sys.exit(1)
    else:
